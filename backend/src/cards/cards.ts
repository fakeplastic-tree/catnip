import { Card } from "@hex-strategy/shared";
...
export const STAGE_1_BUILDING_TEMPLATES = ["litter_box", "scratching_post", "cat_tree", "treat_dispenser", "grooming_station"];
...
export const DEFAULT_DECK = [
  "house_kitten", "house_kitten",
  "stray_kitten", "stray_kitten",
  "tabby", "tom", "alley_cat",
  ...
  "catnip_mist", "laser_pointer", "cardboard_box",
  "purr", "hiss"
];
...
export const CARD_LIBRARY: Record<string, Omit<Card, "id">> = {
  ...
  cat_tree: {
    templateId: "cat_tree",
    name: "Cat Tree",
    description: "a towering monument to feline dominance.",
    type: "building",
    cost: 4,
    target: "tile",
    effects: [{ type: "spawn_building", params: { buildingType: "cat_tree" } }]
  },
  treat_dispenser: {
...
export function createCardInstance(templateId: string): Card {
  ...
  return { ...template, id: `c_${++_cardInstanceId}_${Date.now()}` };
}
